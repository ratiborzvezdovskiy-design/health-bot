...
def give_result(chat_id, score):
    ...
    if score >= 21:
        msg = (
            ...
            "🌿 **Что показал тест:**\n"
            "Ваш организм сейчас работает слаженно — нервная система, пищеварение, суставы и сон держат хороший баланс. Это результат того, что вы уже неплохо заботитесь о себе.\n\n"
            "✨ Именно на этом уровне важнее всего не «лечить», а поддерживать — сохранить этот ресурс надолго и не растерять его на фоне нагрузок и рутины.\n\n"
            "💡 Хотите узнать, как грамотно поддержать организм именно в профилактическом режиме, чтобы это состояние сохранялось как можно дольше?"
        )
    elif score >= 16:
        msg = (
            f"📊 **Ваш результат:** {score} из {max_score} баллов.\n\n"
            "🌿 **Что показал тест:**\n"
            "Организм пока справляется, но уже работает с повышенной нагрузкой. Например: тяжелее просыпаться по утрам, появляется тяжесть после еды, тело быстрее устаёт к вечеру.\n\n"
            "💡 По отдельности каждый из этих признаков кажется мелочью. Но вместе они говорят о том, что организму пора мягко помочь — пока это не переросло в постоянный дискомфорт.\n\n"
            "🌱 В книге я подробно разбираю, с чего начать поддержку именно на этом этапе."
        )
    elif score >= 10:
        msg = (
            f"📊 **Ваш результат:** {score} из {max_score} баллов.\n\n"
            "🌿 **Что показал тест:**\n"
            "Организм уже подаёт сразу несколько сигналов: меньше энергии в течение дня, пищеварение работает нестабильно, появляется скованность в теле, а сон не приносит полного восстановления.\n\n"
            "💬 Большинство людей пытаются лечить каждый симптом отдельно — тяжесть отдельно, усталость отдельно, — хотя чаще всего у них общая причина.\n\n"
            "🌱 В книге я показываю, как эта причина работает и какая последовательность шагов помогает справиться с ней системно, а не точечно."
        )
    else:
        msg = (
            f"📊 **Ваш результат:** {score} из {max_score} баллов.\n\n"
            "🌿 **Что показал тест:**\n"
            "Судя по ответам, организм уже давно работает с перегрузкой сразу в нескольких системах. Постоянная усталость, отсутствие настоящего восстановления и регулярный дискомфорт — это не норма, даже если вы уже привыкли так жить.\n\n"
            "💬 Хорошая новость в том, что чем раньше начать бережное восстановление, тем легче и быстрее возвращается лёгкость в теле.\n\n"
            "🌱 В книге я собрал пошаговый порядок действий — с чего начать уже сегодня и как двигаться дальше."
        )
